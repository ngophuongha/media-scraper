...
describe("MediaController", () => {
  ...
  describe("getMedia", () => {
    it("should call getMedia with parsed parameters", async () => {
      service.getMedia.mockResolvedValue({ data: [], total: 0 });

      await controller.getMedia("2", "50", "video", "search_term");

      expect(service.getMedia).toHaveBeenCalledWith(2, 50, "video", "search_term");
    });

    it("should use default values if parameters are missing", async () => {
      service.getMedia.mockResolvedValue({ data: [], total: 0 });

      await controller.getMedia(undefined as any, undefined as any, undefined as any, undefined as any);

      expect(service.getMedia).toHaveBeenCalledWith(1, 20, undefined, undefined);
    });
  });
});
