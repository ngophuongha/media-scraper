...
describe("useGetMediaList", () => {
  ...
  it("should initialize with default states", () => {
    const mockQueryFn = jest.fn().mockResolvedValue({ page: 1, totalPages: 2, items: [] });
    const { result } = renderHook(
      () => useGetMediaList({ filter: "", debouncedSearch: "", queryFn: mockQueryFn }),
      { wrapper }
    );
  ...
  it("should call queryFn with correct initial params", async () => {
    const mockQueryFn = jest.fn().mockResolvedValue({ page: 1, totalPages: 2, items: [] });
    const { result } = renderHook(
      () => useGetMediaList({ filter: "image", debouncedSearch: "test", queryFn: mockQueryFn }),
      { wrapper }
    );
